...
void wire_grav_maps(void) {
  ...
  AccelSplines[1][1] = neg_plummer; 
      ...
      LatticeForce[0][1] = ewald_force;
      LatticePotential[0][1] = ewald_psi;
      LatticeZero[0][1] = 2.8372975;
      
      LatticeForce[1][0] = ewald_force;
      LatticePotential[1][0] = ewald_psi;
      LatticeZero[1][0] = 2.8372975;
      
      LatticeForce[1][1] = ewald_force;
      LatticePotential[1][1] = ewald_psi;
      LatticeZero[1][1] = 2.8372975;                  
  ...
  GreensFxns[1][1] = pgdelta;
  
  NormedGreensFxns[0][0] = normed_pgdelta;
  NormedGreensFxns[0][1] = normed_pgdelta;
  NormedGreensFxns[1][0] = normed_pgdelta;
  NormedGreensFxns[1][1] = normed_pgdelta;
  ...
  PotentialZero[0][1] = -1 / (sqrt(M_PI) * (((double) ASMTH) / (2*PMGRID)));
  PotentialZero[1][0] = -1 / (sqrt(M_PI) * (((double) ASMTH) / (2*PMGRID))); 
  PotentialZero[1][1] = -1 / (sqrt(M_PI) * (((double) ASMTH) / (2*PMGRID)));     
  
  PotentialFxns[0][0] = newtonian_pot;
  PotentialSplines[0][0] = plummer_pot;
  PotentialFxns[0][1] = newtonian_pot;
  PotentialSplines[0][1] = plummer_pot;
  PotentialFxns[1][0] = newtonian_pot;
  PotentialSplines[1][0] = plummer_pot;  
  PotentialFxns[1][1] = neg_newtonian_pot;
  PotentialSplines[1][1] = neg_plummer_pot;
#endif
...
double newtonian(double target, double source, double h, double r, long N) {
 ...
 // For newtonian acceleration: return source / h; 
 // For dm-dm interaction acceleration: double r_rs_ratio; double beta; r_rs_ratio =  r / All.BoxSize; beta = 1; return beta * exp(-r_rs_ratio) * source / h * (1 + r_rs_ratio);
...
double neg_newtonian(double target, double source, double h, double r, long N) {
  ...
  // Note newtonian does not violate SEP
 // For neg_newtonian acceleration: return -source / h;
 // For dm-dm interaction acceleration: double r_rs_ratio; double beta; r_rs_ratio =  r / All.BoxSize; beta = 1; return source / h * (1 + beta * exp(-r_rs_ratio) * (1 + r_rs_ratio));
 double r_rs_ratio;
 double beta;
 r_rs_ratio =  r / All.BoxSize;
 beta = 1;
 
  return source / h * (1 + beta * exp(-r_rs_ratio) * (1 + r_rs_ratio));
} 
...
double newtonian_pot(double target, double source, double h, double r, long N) {
 ...
 // For newtonian_pot: return source / r; 
 // For dm-dm interaction potential: double r_rs_ratio; double beta; r_rs_ratio =  r / All.BoxSize; beta = 1; return source / r * beta * exp(-r_rs_ratio);
...
double neg_newtonian_pot(double target, double source, double h, double r, long N) {
 ...
 // For neg_newtonian_pot: return -source / r;
 // For dm-dm interaction potential: double r_rs_ratio; double beta; r_rs_ratio =  r / All.BoxSize; beta = 1; return source / r * (beta * exp(-r_rs_ratio) + 1);
 double r_rs_ratio;
 double beta;
 r_rs_ratio = r / All.BoxSize;
 beta = 1;
  return source / r * (beta * exp(-r_rs_ratio) + 1);
}
...
double neg_plummer(double target, double source, double h, double r, long N) {
  ...
  r *= h_inv;
  if(r < 0.5)
    return (1 + beta) * source * h_inv * h_inv * h_inv * 
      (10.666666666667 + r * r * (32.0 * r - 38.4));
  else
    return (1 + beta) * source * h_inv * h_inv * h_inv * 
      (21.333333333333 - 48.0 * r +
...
double neg_plummer_pot(double target, double source, double h, double r, long N) {
  ...
  if(r < 0.5)
    return (1 + beta) * source * h_inv * (-2.8 + r * r * (5.333333333333 + r * r * (6.4 * r - 9.6)));
  else
    return (1 + beta) * source * h_inv * 
      (-3.2 + 0.066666666667 / r + r * r * (10.666666666667 +
					    r * (-16.0 + r * (9.6 - 2.133333333333 * r))));
}
